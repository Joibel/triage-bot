...
// writers to the status file. Readers do not lock; they rely on atomic writes
// (see internal/state) so they never observe a torn file.
//
// Copied from the sibling cherry-picker project, which solves the same problem.
package lockfile
...
//
// The lock is kept on a sidecar file rather than on the status file itself: the
// status file's inode changes on every atomic save (os.Rename), which would
